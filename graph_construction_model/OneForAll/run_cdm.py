...
from OneForAll.gp.lightning.training import lightning_fit, lightning_predict
...
from OneForAll.focal_loss import FocalLoss
...
def predict(params):
    ...
    model_dir = params.model_dir  # "OneForAll/cache_data/my_data_lev_l2lev_n/llama2_13b/graph_model/best.ckpt'
    output_file = params.output_file  # "relation_predictions.txt"
...
def main(params):
    ...
    # num_classes=6
    # loss = FocalLoss(alpha=0.25, gamma=2.0, num_classes=num_classes)
...
def relation_train():
    ...
    configs.append(
        load_yaml(
            os.path.join(
                os.path.dirname(__file__), "OneForAll/configs", "default_config.yaml"
            )
        )
    )
