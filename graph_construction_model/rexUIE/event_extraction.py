...
def event_extraction():
    from rexUIE.data_utils import data_loader, token_config
    from rexUIE.arguments import get_args, DataArguments, UIEArguments
    from rexUIE.model.model import RexModel
    from rexUIE.Trainer.trainer import RexModelTrainer
    from rexUIE.Trainer.utils import compute_metrics
    ...
    if training_args.do_train:
        ...
        eval_dataset = uie_token_data_loader.load_raw_data("eval.json")
    ...
    if training_args.do_train and training_args.do_eval:
        ...
        pred_dataset = uie_token_data_loader.load_raw_data("test.json")
        logger.info('data loading finished')
        if training_args.world_size > 1:
            dist.barrier()
        logger.info('initializing trainer')
