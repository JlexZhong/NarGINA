...
def get_peft_state_maybe_zero_3(named_params, bias):
    ...
    to_return = {key.replace("module.", "", 1) if key.startswith("module.") else key: value 
...
class LLaGATrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:# train的时候会运行
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None

        if self.args.group_by_modality_length:# 默认是false
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.args.lora_enable:
            ...
            if getattr(self.args, "tune_graph_encoder", False):
                graph_encoder_keys_to_match = ['graph_encoder']
                graph_encoder_weight_to_save = get_graph_encoder_state_maybe_zero_3(self.model.named_parameters(), graph_encoder_keys_to_match)
