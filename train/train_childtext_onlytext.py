...
import numpy as np
from ChildText.prompt import GOLD_TEXT,PROMPT_TEMPLATE,PROMPT_TEMPLATE_V1
...
class DataArguments:
    ...
    is_trait_comment: Optional[bool] = field(default=False)
    is_trait: bool = field(default=True)
@dataclass
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "no_graph":
        return preprocess_no_graph(sources, tokenizer)
...
def make_supervised_data_module(
    ...
    rank0_print(colored(train_json[10]['conversations'],'light_yellow'))
...
def compute_qwk(eval_pred):
    ...
    predictions = np.argmax(logits, axis=-1)#(val-dataset-num,3496)

    pred_str = tokenizer.batch_decode(predictions, skip_special_tokens=True)
    labels = np.where(labels == -100, tokenizer.pad_token_id, labels)
    label_str = tokenizer.batch_decode(labels, skip_special_tokens=True)
    rank0_print(pred_str[0][-50:])
    rank0_print(label_str[0])
...
def compute_trait_qwk(eval_pred):
    ...
    # 针对每个维度分别提取并计算 QWK
    for score_type in ["宏观结构得分", "微观叙事得分", "叙事心理描写得分", "总分"]:
        # 提取相应维度的得分
    ...
    for score_type, qwk in qwk_scores.items():
        rank0_print(colored(f"QWK for {score_type}: {qwk}",'light_green'))
...
def _train():
    ...
    rank0_print(colored("Total model params: %.2fM" % (total / 1e6),'light_cyan'))
    rank0_print(colored(f'trainable params: {trainable} || all params: {total} || trainable%: {trainable / total * 100:.3f}','light_cyan'))
    # 构建数据集和数据collator
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    # 训练器
    metric = compute_trait_qwk;rank0_print("使用trait qwk ！！！") if data_args.is_trait_comment or data_args.is_trait else compute_qwk
